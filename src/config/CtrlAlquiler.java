...
public class CtrlAlquiler extends conexion {
    ...
    public boolean buscar(Alquiler alq) {
        ...
        String sql = "SELECT A.cod_Alquiler, A.fecha_inicio_al, A.fecha_final_al, A.dni, O1.nom_Oficina, O2.nom_Oficina, A.num_matricula, A.cod_reserva "
                + "FROM alquiler AS A "
                + "INNER JOIN oficina AS O1 "
                + "ON A.cod_Ofi_1_a = O1.cod_Oficina "
                + "INNER JOIN oficina AS O2 "
                + "ON A.cod_Ofi_2_a = O2.cod_Oficina "
                + "WHERE A.dni = ? AND A.fecha_inicio_al = ?";
        try {
